...
def merge_conversational_chunks(chunks: List[str], min_chunk_size: int = 8) -> Tuple[List[str], Dict[str, Any]]:
    """
...
def semantic_chunker(
    ...
    short_chunks_initial = [chunk for chunk in chunk_texts if len(chunk.split()) <= 7]
    very_short_chunks_initial = [chunk for chunk in chunk_texts if len(chunk.split()) <= 3]
    
    logging.info(f"Initial short chunks (≤7 words): {len(short_chunks_initial)}")
    logging.info(f"Initial very short chunks (≤3 words): {len(very_short_chunks_initial)}")
    ...
    # Apply conversational text merging
    min_chunk_size = max(8, chunk_size // 10)  # Minimum 8 words or 10% of target size
    ...
    short_chunks_final = [size for size in final_chunk_sizes if size <= 7]
    very_short_chunks_final = [size for size in final_chunk_sizes if size <= 3]
    
    logging.info(f"Final chunking results:")
    logging.info(f"  Total chunks: {len(final_chunks)}")
    logging.info(f"  Short chunks (≤7 words): {len(short_chunks_final)} (reduced from {len(short_chunks_initial)})")
    logging.info(f"  Very short chunks (≤3 words): {len(very_short_chunks_final)} (reduced from {len(very_short_chunks_initial)})")
    ...
    if short_chunks_final:
        ...
        for i, chunk in enumerate(final_chunks):
            ...
            if chunk_words <= 7 and i < 5:  # Log first 5 short chunks
