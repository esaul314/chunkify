...
def detect_header_zone(
    ...
    # Build list of pages to check, respecting exclusions
    pages_to_check = [i for i in range(num_pages) if (i + 1) not in exclude_pages]
...
def discover_zones_interactive(
    ...
    # Collect footer data
    footer_y, page_heights, footer_texts_by_y = _collect_zone_data(doc, pages_to_check, "footer")
    ...
    footer_candidates = _analyze_all_zone_candidates(footer_y, footer_texts_by_y, min_count=2)
